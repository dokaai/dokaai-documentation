...
This authentication flow is designed for APIs operating on behalf of an authenticated
customer, such as:
...
For websocket-based customer features such as in-app live notification delivery, use the
same bearer-style token through the Socket.IO `auth.token` field.
